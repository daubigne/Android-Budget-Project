...
/**
 * Data Accessor Object to fetch and save data from database.
...
public class DataAccessor {
	...
	public DataAccessor(Context context, Account account) {
	...
	public void addTransaction(Double amount, Date date, String name, Category category) {
		SQLiteDatabase db = new DatabaseOpenHelper(context)
				.getWritableDatabase();
		...
		this.setAccountBalance(account.getBalance() + amount);
	...
	public void removeTransaction(Transaction transaction) {
		...
		this.setAccountBalance(this.account, this.account.getBalance() - transaction.getAmount());
	}
	...
	public List<Transaction> getTransactions(Account account, SortBy sortBy,
		...
		if(cursor.moveToPosition(start)) {
			for (int i = start; i < Math.min(start+count,cursor.getCount()); i++) {
				...
				Transaction transaction = new Transaction(cursor.getInt(2), (cursor.getInt(3)), date, cursor.getString(2), category);
