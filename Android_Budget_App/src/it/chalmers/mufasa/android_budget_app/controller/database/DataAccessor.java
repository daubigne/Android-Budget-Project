 ...
 * 
 *         A class that helps to translate between the database and the model.
 * 
 */
	...
	public List<Transaction> getTransactions(Account account, SortBy sortBy,
		...
		if (cursor.moveToPosition(start)) {
			for (int i = start; i < Math.min(start + count, cursor.getCount()); i++) {
				...
				Transaction transaction = new Transaction((cursor.getInt(3)),
						date, cursor.getString(2), category, account);
